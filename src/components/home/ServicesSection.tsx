    ...
    <section ref={inViewRef} className={`py-8 md:py-12 lg:py-16 transition-all duration-700 ease-out ${isInView ? "translate-y-0 opacity-100" : "translate-y-6 opacity-0"}`}>
      ...
      <div className="flex w-full gap-3 overflow-x-auto scroll-smooth px-4 pb-6 scrollbar-hide md:gap-4 md:px-8">
        {SERVICES.map((service) => {
          ...
          const className = "relative h-[345px] w-[280px] shrink-0 cursor-pointer overflow-hidden rounded-3xl bg-white shadow-sm transition-shadow duration-300 hover:shadow-lg md:h-[400px] md:w-[320px] lg:h-[462px] lg:w-[370px]";
          ...
          return (
            <div key={service.key} className={className}>
              {content}
            </div>
          );
        })}

...
interface SmartwatchesBrandPageProps {
  params: Promise<{
    locale: string;
    brand: string;
  }>;
...
export default async function SmartwatchesBrandPage({ params }: SmartwatchesBrandPageProps) {
  const { locale, brand } = await params;
  const t = await getTranslations({ locale, namespace: "products" });
  const tCat = await getTranslations({ locale, namespace: "categories" });

  const category = getCategoryBySlug("smartwatches");
  const subcategory = getSubcategoryBySlug("smartwatches", brand);
  ...
  const [modelsResponse, filtersResponse] = await Promise.all([
    getModels({ brand: brandName, category: "wearable", perPage: 50 }),
  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-7xl px-4 py-6">
        <Breadcrumb
          items={[
            { label: tCat("smartwatches"), href: `/${locale}/products/smartwatches` },
            { label: pageTitle },
          ]}
        />

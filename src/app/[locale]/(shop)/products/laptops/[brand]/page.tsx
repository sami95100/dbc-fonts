...
interface LaptopsBrandPageProps {
  params: Promise<{
    locale: string;
    brand: string;
  }>;
...
export default async function LaptopsBrandPage({ params }: LaptopsBrandPageProps) {
  const { locale, brand } = await params;
  const t = await getTranslations({ locale, namespace: "products" });
  const tCat = await getTranslations({ locale, namespace: "categories" });

  const category = getCategoryBySlug("laptops");
  const subcategory = getSubcategoryBySlug("laptops", brand);
  ...
  const [modelsResponse, filtersResponse] = await Promise.all([
    getModels({ brand: brandName, category: "laptop", perPage: 50 }),
  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-7xl px-4 py-6">
        <Breadcrumb
          items={[
            { label: tCat("laptops"), href: `/${locale}/products/laptops` },
            { label: pageTitle },
          ]}
        />
